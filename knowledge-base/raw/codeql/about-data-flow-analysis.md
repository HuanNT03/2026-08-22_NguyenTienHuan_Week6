...
- "[Analyzing data flow in C/C++ ](#analyzing-data-flow-in-cpp)"
- "[Analyzing data flow in C# ](#analyzing-data-flow-in-csharp)"
- "[Analyzing data flow in Go ](#analyzing-data-flow-in-go)"
- "[Analyzing data flow in Java/Kotlin ](#analyzing-data-flow-in-java)"
- "[Analyzing data flow in JavaScript/TypeScript ](#analyzing-data-flow-in-javascript-and-typescript)"
- "[Analyzing data flow in Python ](#analyzing-data-flow-in-python)"
- "[Analyzing data flow in Ruby ](#analyzing-data-flow-in-ruby)"

> [!NOTE]
> Note
>
> Data flow analysis is used extensively in path queries. To learn more about path queries, see "[Creating path queries ](creating-path-queries)."
>

## Data flow graph

The CodeQL data flow libraries implement data flow analysis on a program or function by modeling its data flow graph.
Unlike the [abstract syntax tree ](https://en.wikipedia.org/wiki/Abstract_syntax_tree), the
...
The data flow graph is computed using [classes ](#classes) to model the program elements that represent the graph's nodes.
The flow of data between the nodes is modeled using [predicates ](#predicates) to compute the graph's edges.
