...
"[Analyzing data flow in JavaScript and TypeScript ](analyzing-data-flow-in-javascript-and-typescript)" to check whether there is a path in
...
```
Here is the final query, expressed as a [path query ](#creating-path-queries) so we can examine paths from sources to sinks
...
](https://codeql.github.com/codeql-query-help/javascript/js-prototype-pollution/) query uses two flow states to distinguish completely
tainted objects from partially tainted objects. The [Uncontrolled data used in path expression
](https://codeql.github.com/codeql-query-help/javascript/js-path-injection/) query uses four flow states to track whether a user-controlled
