...
├── .agents/                 # Tài liệu quy chuẩn vận hành và hướng dẫn cho AI Coding Agent
├── .github/                 # CI/CD Workflows tự động kiểm thử và quét an ninh (GitHub Actions)
├── .githooks/               # Git hooks bảo vệ chống rò rỉ khóa bí mật (Gitleaks pre-commit / pre-push)
├── api-server/              # Máy chủ giả lập lỗ hổng (Vulnerable Mock Server) phục vụ kiểm chứng Guardrails
├── configs/                 # Tệp cấu hình các công cụ SAST/DAST (Semgrep, CodeQL, ZAP) và version pinning
├── docker/                  # Dockerfile và cấu hình môi trường container cho các công cụ bảo mật và UI
├── docs/                    # Tài liệu kiến trúc, báo cáo kỹ thuật các tuần và Product Brief
├── frontend/                # Giao diện Web Dashboard Bento Box (Streamlit) và các component giao diện
├── knowledge-base/          # 442+ tài liệu tri thức bảo mật canonical (CWE, OWASP, ASVS) và chỉ mục SQLite/Vector
├── reports/                 # Lưu trữ báo cáo quét thô (raw/), chuẩn hóa (normalized/) và phân tích Agent (analyzed/)
├── schemas/                 # JSON Schema định nghĩa các data contract chuẩn hóa (Unified Findings, Report)
├── scripts/                 # Kịch bản tự động hóa quét SAST/DAST, đối soát benchmark và kiểm tra môi trường
├── src/                     # Toàn bộ mã nguồn cốt lõi của nền tảng Project Sentinel
│   ├── agent/               # Lõi tác nhân ReAct AI Security Agent (ReAct loop, reasoning, tools, prompts)
│   ├── app/                 # Cầu nối (Bridges) kết nối giao diện người dùng với các module backend
│   ├── gateway/             # Tích hợp Kong API Gateway, Safe Requester và chốt chặn Human-in-the-Loop (HITL)
│   ├── guardrails/          # Bộ khiên an toàn 2 chiều (khử khuẩn PII/secrets và phòng chống Prompt Injection)
│   ├── normalizers/         # Bộ chuyển đổi và chuẩn hóa scanner output sang Unified Findings JSONL
│   └── retrieval/           # Công cụ truy hồi tri thức Hybrid (SQLite FTS5 BM25 + Qdrant Dense Vector)
├── target-app/              # Quản lý lock file phiên bản và ứng dụng mục tiêu được cấp phép (OWASP Juice Shop)
├── tests/                   # Bộ kiểm thử tự động toàn diện (unit, integration, contracts, guardrails, agent)
├── docker-compose.yml       # Khai báo dịch vụ Docker cho Target, Scanners và Web UI Dashboard
├── docker-compose.gateway.yml # Khai báo dịch vụ Kong API Gateway cho luồng kiểm thử an toàn
├── Makefile                 # Bộ điều phối lệnh thống nhất cho toàn bộ hệ thống (make help)
├── pyproject.toml           # Khai báo thư viện phụ thuộc Python và cấu hình Ruff, Pytest
└── README.md                # Tài liệu tổng quan dự án, hướng dẫn cài đặt và vận hành full luồng
```

### Các Tệp Quan Trọng Tại Thư Mục Gốc:
- **`README.md`**: Điểm bắt đầu chính của dự án; cung cấp hướng dẫn cài đặt nhanh trong 5 bước, cấu hình biến môi trường `.env`, hướng dẫn chi tiết vận hành full luồng trên Web UI và dòng lệnh CLI.
- **`Makefile`**: Điểm điều phối lệnh tập trung của toàn bộ dự án (`make help` hiển thị đầy đủ danh mục lệnh, ý nghĩa và tham số).
- **`docker-compose.yml` & `docker-compose.gateway.yml`**: Định nghĩa kiến trúc microservices và mạng bảo vệ `sentinel-security` cho ứng dụng mục tiêu, API Gateway, các công cụ quét và Web Dashboard.
- **`pyproject.toml`**: Cấu hình môi trường Python 3.11+, Pydantic v2, Ruff linter và bộ khung kiểm thử Pytest.
