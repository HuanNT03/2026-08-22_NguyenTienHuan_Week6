"""Unit tests for sensitive data redaction in security agent workflows."""
...
def test_redact_secret_tokens() -> None:
    ...
    assert "[REDACTED_JWT]" in redacted or "[REDACTED_SECRET]" in redacted or "[REDACTED_API_KEY]" in redacted
...
def test_redact_dict_recursively() -> None:
    ...
    redacted = mask_sensitive_data(data)
    assert redacted["user"] == "[REDACTED_EMAIL]"
    assert "[REDACTED_" in redacted["nested"]["token"]
