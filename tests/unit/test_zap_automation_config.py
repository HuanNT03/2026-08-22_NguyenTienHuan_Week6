...
ALL_PLANS = (
    "baseline.yaml",
    "baseline-low-memory.yaml",
    "full.yaml",
    "baseline-admin.yaml",
    "baseline-admin-low-memory.yaml",
    "full-admin.yaml",
)
...
def test_all_zap_plans_define_the_exact_protected_target_context() -> None:
    for name in ALL_PLANS:
        ...
        assert context["name"] == "juice-shop"
        assert context["urls"] == [TARGET_URL]
        assert context["includePaths"] == [TARGET_INCLUDE]
        assert context["authentication"]["method"] == "json"
        assert context["authentication"]["parameters"]["loginRequestUrl"] == "http://juice-shop:3000/rest/user/login"
        assert context["sessionManagement"]["method"] == "headers"
        assert context["users"][0]["credentials"]["username"] in ("user@juice-sh.op", "admin@juice-sh.op")
...
def test_spiders_are_explicitly_bound_to_juice_shop() -> None:
    for name in ALL_PLANS:
        ...
        for spider in spiders:
            ...
            assert spider["parameters"]["user"] in ("user", "admin")
    ...
    assert not _jobs(_load_plan("baseline-admin-low-memory.yaml"), "spiderClient")
    assert _jobs(_load_plan("baseline.yaml"), "spiderClient")
    assert _jobs(_load_plan("baseline-admin.yaml"), "spiderClient")


def test_only_full_plan_runs_an_explicitly_scoped_active_scan() -> None:
    for name in ("baseline.yaml", "baseline-low-memory.yaml", "baseline-admin.yaml", "baseline-admin-low-memory.yaml"):
        assert not _jobs(_load_plan(name), "activeScan")

    for name in ("full.yaml", "full-admin.yaml"):
        full_plan = _load_plan(name)
        ...
        assert active_scan[0]["parameters"]["user"] in ("user", "admin")
