#!/usr/bin/env bash
# Run pinned ZAP Baseline with Admin authentication against the running Compose target.
...
ZAP_AUTOMATION_PLAN="$ZAP_CONFIG_DIR/baseline-admin.yaml"
ZAP_AUTOMATION_PLAN_CONTAINER="/zap/configs/baseline-admin.yaml"
HOST_USER="$(id -u):$(id -g)"
...
if (( docker_memory_bytes >= ZAP_MODERN_SPIDER_MIN_BYTES )); then
  ...
  ZAP_AUTOMATION_PLAN="$ZAP_CONFIG_DIR/baseline-admin-low-memory.yaml"
  ZAP_AUTOMATION_PLAN_CONTAINER="/zap/configs/baseline-admin-low-memory.yaml"
...
if [[ -z "$container_id" ]] || [[ "$(docker inspect -f '{{.State.Running}}' "$container_id" 2>/dev/null || true)" != "true" ]]; then
  die "Juice Shop is not running. Run: make target-build && make target-up && make target-wait && make target-smoke && make dast-zap-admin"
fi
...
"$SCRIPT_DIR/write-scan-metadata.sh" --tool zap --scan-profile baseline-admin --report reports/raw/zap.json --base-url http://juice-shop:3000
...
log "ZAP Admin Automation plan: $ZAP_AUTOMATION_PLAN"
docker run --rm \
  --user "$HOST_USER" \
  -e HOME=/tmp \
  -e JAVA_TOOL_OPTIONS=-Duser.home=/tmp \
...
log "ZAP Admin Baseline exit code: $zap_exit_code"
"$SCRIPT_DIR/validate-reports.sh" zap

case "$zap_exit_code" in
  0) log "ZAP Admin Baseline Automation plan completed successfully." ;;
  2) log "ZAP Admin Baseline Automation plan completed with warnings." ;;
  *) die "ZAP Admin Baseline Automation plan failed (exit code $zap_exit_code)." ;;
esac
