"""Parser for local OWASP Top 10:2025 Markdown sources."""
...
_TITLE = re.compile(r"^A(\d{2}):(\d{4})\s+(.+)$", flags=re.IGNORECASE)
...
_OPTIONAL_SECTIONS = tuple(name for name in _SECTION_NAMES if name != "description")
...
def parse_owasp_file(path: Path) -> tuple[KnowledgeDocument, list[str]]:
    ...
    title = _clean_title(first_line.removeprefix("# ").strip())
    title_match = _TITLE.fullmatch(title)
    if title_match is None:
        raise SourceValidationError(f"{path}: invalid OWASP title {title!r}")
    ...
    description = markdown_to_text(sections.get("description", ""))
    if not description:
    ...
    mapped_cwes = sorted(
        {f"CWE-{match}" for match in _CWE.findall(sections.get("list of mapped cwes", ""))},
        key=lambda value: int(value.split("-")[1]),
    )
    content_parts: list[str] = []
    for key, label in _SECTION_NAMES.items():
        if key == "list of mapped cwes":
            value = ", ".join(mapped_cwes)
        else:
            value = markdown_to_text(sections.get(key, ""))
        if value:
            content_parts.append(f"{label}\n{value}")
    aliases = unique_casefold([category_title.strip(), f"OWASP {identifier}", identifier])
    tags = unique_casefold(["owasp", "top-10", *re.findall(r"[a-z0-9]+", category_title.casefold())])
...
def parse_owasp_directory(path: Path) -> tuple[list[KnowledgeDocument], list[str]]:
    """Parse all deterministic OWASP 2025 category paths in a directory."""
    documents: list[KnowledgeDocument] = []
    warnings: list[str] = []
    for source_path in sorted(path.glob("A*_2025-*.md")):
